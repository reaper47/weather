...
from app.models.sampling import (Station, DHT, Temperature, Pressure,
                                 HeatIndex, DS18B20, FC37, TEMT6000, BME280,
                                 Wind, get_samples_for_day, add_new_sample,
                                 find_temperature, find_pressure)
...
from tests.conftest import (A_STATION_NAME, A_TEMPERATURE_C, A_TEMPERATURE_F,
                            ...
                            A_PRESSURE, A_PRESSURE_KPA, A_PRESSURE_MB,
                            A_SPEED_KMPH, A_SPEED_MPH, A_SPEED_MS)
...
def test_format_wind(a_wind):
    """
    WHEN the Wind is converted to a string
    THEN the string is formatted correctly
    """
    format_expected = (f'<Wind #1 -\n'
                       f'  m/s: {a_wind.ms}\n'
                       f'  km/h: {a_wind.kmph}\n'
                       f'  mph: {a_wind.mph}>')

    format_actual = str(a_wind)
...
def test_get_sample_for_day(a_dht_sample):
    ...
    for date in dates:
        sample = DHT(station=a_dht_sample.station, humidity=A_HUMIDITY,
                     temperature=a_dht_sample.temperature,
                     heat_index=a_dht_sample.heat_index, date=date)
        commit(sample)
...
def test_add_new_sample(a_station, a_dht_sample):
    ...
    assert Wind.query.all()[-1] == Wind(ms=20.54, kmph=88.25, mph=66.22, date=date)
